...
func TestUploadFlowUsesAssetUploadURLForGenericFiles(t *testing.T) {
	t.Parallel()

	filePath := filepath.Join(t.TempDir(), "report.pdf")
	fileBody := []byte("%PDF-1.1\nfixture")
	if err := os.WriteFile(filePath, fileBody, 0o644); err != nil {
		t.Fatalf("write fixture: %v", err)
	}
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/upload/policies/assets":
			...
			_ = json.NewEncoder(w).Encode(map[string]any{
				"upload_url":       server.URL + "/s3",
				"asset_upload_url": "/upload/repository-files/9",
				"asset": map[string]any{
					"id":           9,
					"name":         file.Name,
					"size":         file.Size,
					"content_type": file.ContentType,
					"href":         "https://github.com/user-attachments/files/9/report.pdf",
				},
				"form": map[string]string{
					"key":              "key",
					"acl":              "private",
		...
		case r.Method == http.MethodPost && r.URL.Path == "/s3":
			w.WriteHeader(http.StatusNoContent)
		case r.Method == http.MethodPut && r.URL.Path == "/upload/repository-files/9":
			if err := r.ParseMultipartForm(1 << 20); err != nil {
				t.Fatalf("ParseMultipartForm() error = %v", err)
			}
			if got := r.FormValue("authenticity_token"); got != "finalize-token" {
	...
	if result.URL != "https://github.com/user-attachments/files/9/report.pdf" {
		t.Fatalf("Upload() url = %q", result.URL)
	}
	if result.Category != CategoryFile {
		t.Fatalf("Upload() category = %q, want %q", result.Category, CategoryFile)
	}
}
